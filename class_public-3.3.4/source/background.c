...
)
{
  ...
  if (pba->fluid_equation_of_state == GREA)
  {
    pvecback[pba->index_bg_H] = sqrt(rho_tot);
    /** - in GREA the curvature is encoded in the entropic DE term, not as an
        explicit Omega_k a^-2 in the Friedmann equation */
    ...
    double *integral_fld)
{
  ...
  case GREA:
  {
    int idx = pba->grea_last_index;
    if (idx < 0)
      idx = 0;
    if (idx > pba->grea_bt_size - 2)
      idx = pba->grea_bt_size - 2;
    if (a <= 0.)
    {
      idx = 0;
    }
    ...
    {
      double loga = log(a);
      if (loga <= pba->grea_loga_table[0])
      {
        idx = 0;
      }
      else
      {
        while ((idx < pba->grea_bt_size - 2) && (loga > pba->grea_loga_table[idx + 1]))
          idx++;
        while ((idx > 0) && (loga < pba->grea_loga_table[idx]))
          idx--;
      }
    }
    pba->grea_last_index = idx;
    {
      double x0 = pba->grea_loga_table[idx];
      double x1 = pba->grea_loga_table[idx + 1];
      double t = 0.;
      if (a > 0.)
      {
        double loga = log(a);
      ...
      double w0 = pba->grea_w_table[idx];
      double w1 = pba->grea_w_table[idx + 1];
      *w_fld = w0 + t * (w1 - w0);
    }
  }
  break;
  ...
  case GREA:
  {
    ...
    pba->grea_last_index = idx;
    {
      ...
      double dw0 = pba->grea_dw_over_da_table[idx];
      double dw1 = pba->grea_dw_over_da_table[idx + 1];
      *dw_over_da_fld = dw0 + t * (dw1 - dw0);
    }
  }
  break;
  }
  ...
  case GREA:
  {
    ...
    pba->grea_last_index = idx;
    {
      double x0 = pba->grea_loga_table[idx];
      double x1 = pba->grea_loga_table[idx + 1];
      double t = (loga - x0) / (x1 - x0);
      double I0 = pba->grea_integral_fld_table[idx];
      double I1 = pba->grea_integral_fld_table[idx + 1];
      *integral_fld = I0 + t * (I1 - I0);
    }
  }
  }
    ...
    struct background *pba)
{
  ...
  /** - if GREA is selected, precompute the fluid table and rescale H0/densities */
  if (pba->fluid_equation_of_state == GREA)
  {
    class_call(background_grea_precompute(ppr, pba),
               pba->error_message,
               pba->error_message);
  }
    ...
    struct background *pba)
{
  ...
  if (pba->grea_loga_table != NULL)
    free(pba->grea_loga_table);
  if (pba->grea_tau_table != NULL)
    free(pba->grea_tau_table);
  if (pba->grea_E_table != NULL)
    free(pba->grea_E_table);
  if (pba->grea_dE_dloga_table != NULL)
    free(pba->grea_dE_dloga_table);
  if (pba->grea_w_table != NULL)
    free(pba->grea_w_table);
  if (pba->grea_dw_over_da_table != NULL)
    free(pba->grea_dw_over_da_table);
  if (pba->grea_integral_fld_table != NULL)
    free(pba->grea_integral_fld_table);
...
int background_grea_precompute(
    struct precision *ppr,
    struct background *pba)
{

  int i;
  double a_ini, loga_ini, dloga, a, tau, E, fG, W, Omega_r0, Omega_m0;
  double g, alpha, integral;
  double *tau_table = NULL;
  double *E_table = NULL;
  double *w_table = NULL;
  double *dw_over_da_table = NULL;
  double *integral_table = NULL;
  double *loga_table = NULL;
  double *dE_dloga_table = NULL;
  double *tau_old = NULL;
  int N;

  if (pba->sqrt_k_eta0 <= 0.)
  {
    class_stop(pba->error_message,
               "GREA requires a positive parameter sqrt_k_eta0");
  }

  N = ppr->background_Nloga;
  a_ini = ppr->a_ini_over_a_today_default;
  loga_ini = log(a_ini);
  dloga = -loga_ini / (double)(N - 1);

  class_alloc(loga_table, N * sizeof(double), pba->error_message);
  class_alloc(tau_table, N * sizeof(double), pba->error_message);
  class_alloc(E_table, N * sizeof(double), pba->error_message);
  class_alloc(w_table, N * sizeof(double), pba->error_message);
  class_alloc(dw_over_da_table, N * sizeof(double), pba->error_message);
  class_alloc(integral_table, N * sizeof(double), pba->error_message);
  class_alloc(dE_dloga_table, N * sizeof(double), pba->error_message);

  Omega_r0 = pba->Omega0_g;
  Omega_r0 += pba->Omega0_ur;
  Omega_r0 += pba->Omega0_idr;
  Omega_m0 = pba->Omega0_b;
  Omega_m0 += pba->Omega0_cdm;
  Omega_m0 += pba->Omega0_idm;
  Omega_m0 += pba->Omega0_ncdm_tot;

  g = pba->sqrt_k_eta0;
  W = _PI_ * (sinh(2. * g) - 2. * g);

  tau = a_ini / sqrt(Omega_r0);
  for (i = 0; i < N; ++i)
  {
    loga_table[i] = loga_ini + i * dloga;
    if (i == 0)
    {
      tau_table[i] = tau;
    }
    ...
    {
      double loga_half = loga_table[i - 1] + 0.5 * dloga;
      double a0 = exp(loga_table[i - 1]);
      double a1 = exp(loga_half);
      double a2 = exp(loga_table[i]);
      double k1, k2, k3, k4;
      ...
      OmegaGB = (4. * _PI_ / 3.) / W;
      x = 2. * tau_table[i - 1];
      k1 = 1. / (a0 * sqrt(Omega_m0 / pow(a0, 3) + Omega_r0 / pow(a0, 4) + OmegaGB * sinh(x) / (a0 * a0)));
      tau1 = tau_table[i - 1] + 0.5 * dloga * k1;
      x = 2. * tau1;
      k2 = 1. / (a1 * sqrt(Omega_m0 / pow(a1, 3) + Omega_r0 / pow(a1, 4) + OmegaGB * sinh(x) / (a1 * a1)));
      tau2 = tau_table[i - 1] + 0.5 * dloga * k2;
      x = 2. * tau2;
      k3 = 1. / (a1 * sqrt(Omega_m0 / pow(a1, 3) + Omega_r0 / pow(a1, 4) + OmegaGB * sinh(x) / (a1 * a1)));
      tau3 = tau_table[i - 1] + dloga * k3;
      x = 2. * tau3;
      k4 = 1. / (a2 * sqrt(Omega_m0 / pow(a2, 3) + Omega_r0 / pow(a2, 4) + OmegaGB * sinh(x) / (a2 * a2)));
      tau_table[i] = tau_table[i - 1] + (dloga / 6.) * (k1 + 2. * k2 + 2. * k3 + k4);
    }
  }
  ...
  for (i = 0; i < N; ++i)
  {
    a = exp(loga_table[i]);
    tau = tau_table[i];
    fG = (4. * _PI_ / 3.) * sinh(2. * tau) / (W * a * a);
    E = sqrt(Omega_m0 / pow(a, 3) + Omega_r0 / pow(a, 4) + fG);
    E_table[i] = E;
  }

  pba->grea_E1 = E_table[N - 1];
  for (i = 0; i < N; ++i)
  {
    E_table[i] /= pba->grea_E1;
  }
  ...
  for (i = 0; i < N; ++i)
  {
    a = exp(loga_table[i]);
    tau = tau_table[i];
    E = E_table[i] * pba->grea_E1;   /* physical E: undo normalization for w formula */
    w_table[i] = -(1. / 3.) * (2. * a / (a * a * E) / tanh(2. * tau) + 1.);
  }
  ...
  for (i = 0; i < N; ++i)
  {
    if (i == 0)
    {
      dE_dloga_table[i] = (E_table[1] - E_table[0]) / (loga_table[1] - loga_table[0]);
    }
    else if (i == N - 1)
    {
      dE_dloga_table[i] = (E_table[N - 1] - E_table[N - 2]) / (loga_table[N - 1] - loga_table[N - 2]);
    }
    else
    {
      dE_dloga_table[i] = (E_table[i + 1] - E_table[i - 1]) / (loga_table[i + 1] - loga_table[i - 1]);
    }
  }
  ...
  for (i = 0; i < N; ++i)
  {
    if (i == 0)
    {
      dw_over_da_table[i] = (w_table[1] - w_table[0]) / (exp(loga_table[1]) - exp(loga_table[0]));
    }
    else if (i == N - 1)
    {
      dw_over_da_table[i] = (w_table[N - 1] - w_table[N - 2]) / (exp(loga_table[N - 1]) - exp(loga_table[N - 2]));
    }
    else
    {
      dw_over_da_table[i] = (w_table[i + 1] - w_table[i - 1]) / (exp(loga_table[i + 1]) - exp(loga_table[i - 1]));
    }
  }

  integral_table[N - 1] = 0.;
  for (i = N - 2; i >= 0; --i)
  {
    integral_table[i] = integral_table[i + 1] + 0.5 * (3. * (1. + w_table[i]) + 3. * (1. + w_table[i + 1])) * (loga_table[i + 1] - loga_table[i]);
  }

  pba->grea_bt_size = N;
  pba->grea_loga_table = loga_table;
  pba->grea_tau_table = tau_table;
  pba->grea_E_table = E_table;
  pba->grea_dE_dloga_table = dE_dloga_table;
  pba->grea_w_table = w_table;
  pba->grea_dw_over_da_table = dw_over_da_table;
  pba->grea_integral_fld_table = integral_table;
  pba->grea_last_index = 0;

  pba->grea_alpha = pba->sqrt_k_eta0 / (pba->grea_E1 * tau_table[N - 1]);
  pba->h = pba->H0 * _c_ / 1.e5;

  /* Set the GREA fluid density so that the budget closes and standard
     matter/radiation densities remain fixed. */
  pba->Omega0_fld = 1. - Omega_m0 - Omega_r0;
    ...
    struct background *pba)
{
  ...
  if (pba->background_verbose > 1)
  {
    ...
    if (pba->fluid_equation_of_state == GREA)
    {
      printf(" GREA diagnostics: sqrt_k_eta0 = %-15g, E(1) = %-15g, alpha = %-15g\n", pba->sqrt_k_eta0, pba->grea_E1, pba->grea_alpha);
      printf("                 H0_input = %-15g (Mpc^-1), H0_physical = %-15g (Mpc^-1), h = %-15g\n", pba->H0_grea, pba->H0, pba->h);
    }
    printf(" -------------------------------------------------------------------- \n");
  }
