                                  ...
                                  ErrorMsg errmsg)
{
  ...
  /** 6.a) GREA curvature scale parameter */
  /* Read */
  /* This is read only when GREA is selected in the equation-of-state block below. */
  ...
  if (pba->fluid_equation_of_state == GREA)
  {
    class_test((flag1 == _TRUE_) || (flag2 == _TRUE_),
               errmsg,
               "For fluid_equation_of_state = GREA you must not specify Omega_Lambda or Omega_fld explicitly.");
  }

  /* Step 2 */
  if ((flag1 == _FALSE_) && (pba->fluid_equation_of_state != GREA))
  {
    /* Fill with Lambda */
  ...
  /** 8.a) If Omega fluid is different from 0 or if the GREA model is selected */
  {
    ...
    if (flag1 == _TRUE_)
    {
      ...
      else if ((strstr(string1, "GREA") != NULL) || (strstr(string1, "grea") != NULL))
      {
        pba->fluid_equation_of_state = GREA;
      }
      else
      {
    ...
    if (pba->fluid_equation_of_state == GREA)
    {
      /** 8.a.2.4) Equation of state of the fluid in 'GREA' case */
      /* Read */
      class_read_double("cs2_fld", pba->cs2_fld);
      class_read_double("sqrt_k_eta0", pba->sqrt_k_eta0);
      class_test(pba->sqrt_k_eta0 <= 0.,
                 errmsg,
                 "sqrt_k_eta0 must be positive for GREA");
      pba->use_ppf = _TRUE_; /* mandatory for phantom crossing and stability */
      pba->has_fld = _TRUE_;
    }
  }
                         ...
                         struct output *pop)
{
  ...
  pba->grea_E1 = 0.;
  pba->grea_bt_size = 0;
  pba->grea_loga_table = NULL;
  pba->grea_tau_table = NULL;
  pba->grea_E_table = NULL;
  pba->grea_dE_dloga_table = NULL;
  pba->grea_w_table = NULL;
  pba->grea_dw_over_da_table = NULL;
  pba->grea_integral_fld_table = NULL;
